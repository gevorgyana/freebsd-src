...
/*
 * Set on nodes conjured into a master tdata_t by conjure_template() in
 * merge.c.  tdesc_free_cb() will decrement t_refcnt instead of freeing
 * immediately; the node is freed only when the count reaches zero.  This
 * allows per-CU tdata_t structures to be destroyed after each merge
 * without corrupting the master type graph.
 */
 ...
 *
 * These are placed here so that tdesc_free() is already declared above.
